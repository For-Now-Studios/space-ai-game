		...
		for(CharacterObject *c : characters) {
			...
			if (arrived && c->currentTask != nullptr) {
					...
					if (checkStillAlive(characters,
						...
						if (whichRoom(&currClick.rooms,
							...
							if(c->currentTask->tolerance
										== 0){
								printf("%s removed a task after waiting for too long! It's sister task was %p\n", c->name, c->currentTask->sisterTask);
								c->currentTask->
									waitingFor->
									removeTask(c->
									currentTask->
				...
				if (c->currentTask->waitTime < 1) {
					printf("%s completed their task!\n", c->name);
					if (c->currentTask->function != nullptr) {
						c->currentTask->function(
								c->currentTask->data);
					}

					//If this was a relationship task
					if(c->currentTask->sisterTask != nullptr){
						Task *t = c->currentTask->sisterTask;
						if(t->function != nullptr){
							t->function(t->data);
						}
						c->currentTask->waitingFor->removeTask(
										t);
					}
